...
def main_ae(signals, targets, data):
    ...
    axs.plot(history.history['loss'], label='Training Loss', color='blue', linestyle='--')
    if 'val_loss' in history.history:
        axs.plot(history.history['val_loss'], label='Validation Loss', color='green', linestyle='-.')
    ...
    plt.figure(figsize=(12, (nr_true_col + 1) * 2))
    mp = -1
    ...
    for index, signal in enumerate(all_columns):
        ...
        plt.plot(du.iloc[:, index], '-', linewidth=1, color='magenta', label=f'{signal} true')
        plt.plot(dpred.iloc[:, index], '--', linewidth=1, color='teal', label=f'{signal} autoencoded')
        plt.title(f'{signal} True vs Prediction')
        plt.xlabel('Date')
        plt.ylabel(signal)
        plt.legend()
        plt.grid(True)
    ...
    plt.plot(pd.DataFrame(middle[:, mp, :], index=df.index[lb - 1:]), marker='o', linestyle='-', color='purple')
    plt.title('Middle Layer Representation')
    plt.xlabel('Date')
    plt.ylabel('Middle Layer Value')
