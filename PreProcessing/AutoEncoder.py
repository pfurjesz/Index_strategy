...
def main_ae(signals, targets, data):
    ...
    axs.plot(history.history['loss'], color='blue')
    ...
    colors = ['blue', 'red', 'green', 'orange', 'purple', 'cyan', 'magenta', 'yellow', 'black', 'gray', 'brown', 'pink', 'lime', 'coral', 'teal']

    for index, signal in enumerate(all_columns):
        signal = signal.lstrip('_')
        plt.subplot(nr_true_col + 1, 1, index + 1)
        plt.plot(du.iloc[:, index], '-', linewidth=1, color=colors[index % len(colors)])
        plt.plot(dpred.iloc[:, index], '--', linewidth=1, color=colors[index % len(colors)])
        plt.legend([f'{signal} true', f'{signal} autoencoded'])

    middle = model.encoder(u_true)
    plt.subplot(nr_true_col + 1, 1, nr_true_col + 1)
    plt.plot((pd.DataFrame(middle[:, mp, :], index=df.index[lb - 1:])), color='magenta')
    ...
    dpred = dpred.iloc[:, :-1]
    dat['Date'] = date4Fig
    #out=pd.merge(dpred, dat.iloc[lb - 1:, :], left_index=True, right_index=True)
    return [dpred,dat]
