...
def predictor_module(data, model_names, target_variable, output_dir):
    ...
    # Ensure the Date index is set for the DataFrame
    ...
    predictions_full_df = pd.DataFrame(index=data.index)
    predictions_train_df = pd.DataFrame(index=data.index[:split_idx])
    predictions_test_df = pd.DataFrame(index=data.index[split_idx:])
    ...
    plt.plot(data.index, y_scaled_inverse, label='Original Data', color='black', linestyle='--', linewidth=1.5)
    colors = ['blue', 'green', 'red', 'purple']
    for i, name in enumerate(model_names):
        plt.plot(data.index, predictions_full_df[name], label=f'{name} Predictions', color=colors[i], linewidth=1.5)
    ...
    plt.figure(figsize=(14, 10))
    plt.plot(data.index[:split_idx], y_train_inverse, label='Original Data', color='black', linestyle='--', linewidth=1.5)
    for i, name in enumerate(model_names):
        plt.plot(data.index[:split_idx], predictions_train_df[name], label=f'{name} Predictions', color=colors[i], linewidth=1.5)
    ...
    plt.figure(figsize=(14, 10))
    plt.plot(data.index[split_idx:], y_test_inverse, label='Original Data', color='black', linestyle='--', linewidth=1.5)
    for i, name in enumerate(model_names):
        plt.plot(data.index[split_idx:], predictions_test_df[name], label=f'{name} Predictions', color=colors[i], linewidth=1.5)
    ...
    predictions_full_df.to_csv(os.path.join(output_dir, 'full_predictions.csv'), index=True)
    predictions_train_df.to_csv(os.path.join(output_dir, 'train_predictions.csv'), index=True)
    predictions_test_df.to_csv(os.path.join(output_dir, 'test_predictions.csv'), index=True)
